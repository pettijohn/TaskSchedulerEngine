...
## Project Context
- **Tech Stack:** C# with nullable reference types; SDK-style .NET library targeting .NET Core 3.1, .NET 6.0, and .NET 7.0; .NET 6 Worker Service sample using Microsoft.Extensions.Hosting 6.0.0; MSTest 2.1.0 with Microsoft.NET.Test.Sdk 16.5.0 and Coverlet 1.x/3.1.0; no database or external runtime dependency.
...
- **Development Server:** `DOTNET_ROLL_FORWARD=Major dotnet run --project sample/sample.csproj --framework net6.0`
...
- `sample/` and `test/` — .NET 6 hosted-worker usage example and the .NET 7 MSTest suite covering rule parsing/evaluation, threading, shutdown, expiration, timezones, and retries.
...
- Preserve compatibility across `netcoreapp3.1`, `net6.0`, and `net7.0`; APIs such as source-generated regex require conditional compilation. The devcontainer installs only .NET SDK/runtime 10, so `DOTNET_ROLL_FORWARD=Major` is required to run the net6.0 sample or net7.0 tests there.
